              ...
              >
                <div className={`${isFeatured ? "h-64 md:h-80" : "h-48"} bg-gray-100 dark:bg-slate-800 relative overflow-hidden`}>
                  ...
                  {news.imagem ? (
                    ...
                    <img src={news.imagem} alt={news.titulo} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-gray-400 font-medium text-lg bg-gradient-to-br from-gray-100 to-gray-200 dark:from-slate-800 dark:to-slate-900">
                      BICT News
                    </div>
                  )}
                </div>
                <div className={`p-6 flex-1 flex flex-col ${isFeatured ? "md:p-8" : ""}`}>
                  <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-3">
                    ...
                    {news.dataPublicacao ? new Date(news.dataPublicacao).toLocaleDateString('pt-BR') : 'Sem data'}
                  ...
                  <h3 className={`${isFeatured ? "text-2xl md:text-3xl" : "text-xl"} font-bold text-gray-900 dark:text-white mb-3 group-hover:text-primary transition-colors tracking-tight`}>
                    {news.titulo}
                  </h3>
